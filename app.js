...
class HomeManagerApp {
    ...
    showPinSetup() {
        ...
        modal.className = 'pin-setup-overlay';
        modal.innerHTML = `
            <div class="pin-setup-modal">
                <div class="pin-setup-header">
                    <h3>Create PIN</h3>
                    <p>Enter a 4-digit PIN to secure your app</p>
                </div>
                ...
                <div class="pin-keypad">
                    ...
                    <button class="pin-key pin-key-backspace" id="pin-backspace">
                ...
                <div class="pin-actions">
                    <button class="pin-cancel-btn" onclick="this.closest('.pin-setup-overlay').remove()">Cancel</button>
                    <button class="pin-confirm-btn" id="pin-confirm-btn" disabled>Confirm PIN</button>
                </div>
            </div>
        `;
    ...
    initPinSetup(modal) {
        ...
        const keys = modal.querySelectorAll('.pin-key:not(.pin-key-empty)');
        ...
        confirmBtn.addEventListener('click', () => {
            ...
            if (step === 'create') {
                ...
                step = 'confirm';
                modal.querySelector('.pin-setup-header h3').textContent = 'Confirm PIN';
                modal.querySelector('.pin-setup-header p').textContent = 'Enter your PIN again to confirm';
                updateConfirmBtn();
                updateDots();
            } else {
                // Check if PINs match
                if (pin === confirmPin) {
                    ...
                    confirmPin = '';
                    step = 'create';
                    modal.querySelector('.pin-setup-header h3').textContent = 'Create PIN';
                    modal.querySelector('.pin-setup-header p').textContent = 'Enter a 4-digit PIN to secure your app';
                    updateConfirmBtn();
                    updateDots();
                }
            }
    ...
    showPinEntry() {
        const modal = document.createElement('div');
        modal.className = 'pin-entry-overlay';
        modal.innerHTML = `
            <div class="pin-entry-modal">
                ...
                <div class="pin-keypad">
                    ...
                    <button class="pin-key pin-key-empty"></button>
                    <button class="pin-key" data-value="0">0</button>
                    <button class="pin-key pin-key-backspace" id="pin-entry-backspace">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M3 6h18"/>
                            <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/>
                            <path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/>
                ...
                <div class="pin-actions">
                    <button class="pin-forgot-btn" onclick="app.forgotPin()">Forgot PIN?</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
                        ...
                        setTimeout(() => {
                            if (enteredPin === storedPin) {
                                ...
                                if (attempts >= maxAttempts) {
                                    this.showToast('Too many failed attempts. App locked for 5 minutes.', 'error');
                                    setTimeout(() => {
                                        location.reload();
                                    }, 300000); // 5 minutes
                                } else {
    ...
    forgotPin() {
        // For security, we can't recover PIN. User must reset.
        if (confirm('Forgot your PIN? This will disable PIN lock and you\'ll need to set a new one.')) {
            localStorage.removeItem('app_pin');
            localStorage.setItem('pin_lock_enabled', 'false');
            document.getElementById('pin-lock-toggle').checked = false;
            this.showPinOptions(false);
            this.updatePinStatus();
            this.showToast('PIN lock disabled. You can set a new PIN in settings.', 'success');
            document.querySelector('.pin-entry-overlay').remove();
        }
    }
    ...
    showPinVerification(reason) {
        const modal = document.createElement('div');
        modal.className = 'pin-verify-overlay';
        modal.innerHTML = `
            <div class="pin-verify-modal">
                <div class="pin-verify-header">
                    <h3>Verify Current PIN</h3>
                    <p>Enter your current PIN to ${reason === 'change' ? 'change it' : 'disable PIN lock'}</p>
                </div>
                <div class="pin-input-container">
                    <div class="pin-dots">
                        <div class="pin-dot" data-index="0"></div>
                ...
                <div class="pin-keypad">
                    ...
                    <button class="pin-key pin-key-empty"></button>
                    <button class="pin-key" data-value="0">0</button>
                    <button class="pin-key pin-key-backspace" id="pin-verify-backspace">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M3 6h18"/>
                            <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/>
                            <path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/>
                ...
                <div class="pin-actions">
                    <button class="pin-cancel-btn" onclick="this.closest('.pin-verify-overlay').remove()">Cancel</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    setupSwipeGestures() {
        ...
        // PIN Security: Lock when page unloads (for non-PWA)
    ...
    handleFormSubmit(form) {
        ...
        if (!id) {
            ...
            // but do it after navigation to avoid conflicts
    ...
    confirmClearRecentUpdates() {
        ...
        dialog.innerHTML = `
            <div class="delete-confirm-dialog">
                <button class="delete-confirm-close-btn" onclick="this.closest('.delete-confirm-overlay').remove()">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <line x1="18" y1="6" x2="6" y2="18"/>
                        <line x1="6" y1="6" x2="18" y2="18"/>
                    </svg>
    ...
    renderHome() {
        ...
        // Calculate savings balance from both savings category and ALL financial entries
        ...
        const savingsBalanceFromFinances = finances
            .reduce((sum, f) => {
                ...
                // All financial entries with positive amounts contribute to savings
                return sum + amount;
            }, 0);
